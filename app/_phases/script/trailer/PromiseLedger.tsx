...
export default function PromiseLedger({
  ...
  return (
    <section data-testid="promise-ledger" className="rounded-2xl border border-white/8 bg-white/[0.02] p-4">
      ...
      {rows.length === 0 ? (
        <p className="font-jetbrains mt-3 text-[11px] text-white/40">
          no promise declared on any beat — the ledger is empty, which is not the same as the cut promising nothing
        </p>
      ) : (
        <ul className="mt-3 space-y-2.5">
          {rows.map(({ beat, p }) => (
            <li key={p.id} data-testid={`promise-${p.id}`} className="text-[12px] leading-snug">
              <p className="font-hanken text-[14px] text-slate-200">{p.sentence}</p>
              <p className="font-jetbrains mt-0.5 text-[10px] tracking-[0.1em] text-white/30">
                by {p.source} · on “{beat.label}” @{beat.at}
                {p.grade && <span className="ml-2 text-white/45">graded {p.grade}</span>}
              </p>
              <div className="mt-1 flex items-center gap-2">
                <label className="font-jetbrains text-[10px] tracking-[0.14em] text-white/35 uppercase" htmlFor={`payer-${p.id}`}>
                  payer
                </label>
                <input
                  id={`payer-${p.id}`}
                ...
                <span
                  ...
                  className={`font-jetbrains shrink-0 text-[10px] tracking-[0.1em] ${p.payer ? "text-emerald-300/80" : "text-amber-200"}`}
                >
      ...
      <div className="mt-4 border-t border-white/8 pt-3">
        <p className="font-jetbrains text-[10px] tracking-[0.14em] text-white/35 uppercase">add a promise</p>
        <div className="mt-1.5 flex flex-wrap gap-1.5">
          <select
            ...
            className="font-jetbrains rounded-lg border border-white/12 bg-white/[0.03] px-2 py-1 text-[11px] text-white/75"
          >
