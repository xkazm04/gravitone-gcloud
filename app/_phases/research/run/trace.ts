...
// `OUTCOMES` LIVED HERE and is gone (2026-09-08). It was the label/hint table
// for `OutcomePicker` — the "prototype · drive the ending" pills — and the
// operator's ruling is that development affordances leave the product. With the
// picker deleted the table had no reader, and `OUTCOMES_ARE_EXHAUSTIVE` with it:
// its whole job was to fail the build when a fourth `RunOutcome` was added and
// the PILLS did not grow a fourth entry. `STOP_AT` in useResearchRun.ts is still
// `Record<RunOutcome, number>`, so the union is still held to a table that has
// to grow with it — the check that guarded the engine survives; the one that
// guarded the control went with the control.
