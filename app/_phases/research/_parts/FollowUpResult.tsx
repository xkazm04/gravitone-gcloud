...
import type { Effect, FollowUpRequest } from "../followup";
...
export default function FollowUpResult({ result }: { result: NonNullable<FollowUpRequest["result"]> }) {
  return (
    <div className="mt-3 border-t border-white/8 pt-3">
      ...
      <p className="font-jetbrains mt-2.5 text-[10px] text-white/28">
        {result.sources.length} source{result.sources.length === 1 ? "" : "s"} · applying these writes a
        new notebook revision — the current script is unaffected
      </p>
    </div>
  );
}
