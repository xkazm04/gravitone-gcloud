 ...
 * cursor-anchored source chip, the breadcrumb HUD, and the nav-mode hint. Kept
 * separate so the inspector component stays focused on state and wiring.
 *
 * ── ON THE COLOUR-LITERAL RULE ──────────────────────────────────────────────
 * `components/ui/tokens.ts` is the only place that may declare a colour used to
 * draw the app's CHROME. This file is not chrome — it is a DEVTOOLS SKIN, and it
 * is deliberately fixed. It has to stay legible while you are debugging the
 * theme itself (including a half-broken one, and including surfaces that
 * deliberately render without <GravitoneTokens>, like app/global-error.tsx), so
 * it must not follow the app's variables. Same shape as the exception tokens.ts
 * already grants global-error.tsx. It is dev-only and never reaches a production
 * bundle — see the gate in DevInspector.tsx and app/layout.tsx.
 */
...
const ACCENT = "#38bdf8"; // cyan — the inspected element
const DIM = "#a855f7"; // purple — the element actually under the pointer
const OK = "#34d399"; // green — copy confirmation

/** Split `app/a/b/File.tsx:88` → `{ dir: 'app/a/b/', file: 'File.tsx:88' }`. */
...
/** A compact `File.tsx:line` chip pinned to the element under the cursor. */
  ...
  return (
    <div data-devinspector style={PANEL}>
      ...
      {copied ? (
        ...
        // The honest empty state: the overlay is mounted but nothing stamped the
        // DOM, so it can only say how to turn stamping on.
